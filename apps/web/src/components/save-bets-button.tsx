...
// `strategy` descreve como os jogos foram gerados (critério, redução, fixas…)
// e aparece em Minhas apostas.
export function SaveBetsButton({ slug, tickets, name, strategy, className = "" }: { slug: LotterySlug; tickets: BacktestTicket[]; name: string; strategy: string; className?: string }) {
  const [state, setState] = useState<{ kind: "idle" | "saving" } | { kind: "saved"; target: number } | { kind: "error"; message: string }>({ kind: "idle" });

  async function save() {
    setState({ kind: "saving" });
    try {
      const response = await fetch("/api/apostas", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slug, name, mode: strategy, tickets }) });
  ...
  return <span className={`save-bets ${className}`}>
    <button type="button" className="save-bets-button" disabled={state.kind === "saving"} onClick={save}>
      ...
      {state.kind === "saving" ? "Salvando…" : `Salvar ${tickets.length} ${tickets.length === 1 ? "jogo" : "jogos"} para conferir`}
    </button>
