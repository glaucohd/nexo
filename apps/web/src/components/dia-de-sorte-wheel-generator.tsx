...
type Guarantee = 4 | 5;
type Round = { id: number; pool: number[]; guarantee: Guarantee; tickets: DiaDeSorteWheelTicket[]; copied: boolean };

const guarantees: { value: Guarantee; label: string; poolSizes: number[] }[] = [
  { value: 4, label: "Garante 4 pontos", poolSizes: [8, 9, 10, 11, 12, 13, 14] },
  { value: 5, label: "Garante 5 pontos", poolSizes: [8, 9, 10, 11, 12, 13] },
];

export function DiaDeSorteWheelGenerator({ history }: { history: DrawNumbers[] }) {
  const [guarantee, setGuarantee] = useState<Guarantee>(4);
  ...
  const poolSizes = guarantees.find((entry) => entry.value === guarantee)?.poolSizes ?? [];
  ...
  function chooseGuarantee(next: Guarantee) {
    const nextPoolSizes = guarantees.find((entry) => entry.value === next)?.poolSizes ?? [];
    setGuarantee(next);
    if (!nextPoolSizes.includes(poolSize)) setPoolSize(nextPoolSizes[0]);
    setPool([]);
    setError(null);
  }
  ...
  async function generate() {
    setBusy(true);
    try {
      const tickets = diaDeSorteWheel(pool, guarantee);
      setRounds((current) => [{ id: Date.now(), pool: [...pool].sort((a, b) => a - b), guarantee, tickets, copied: false }, ...current]);
      setPool([]);
      setError(null);
    } catch (cause) {
  ...
  return <main className={styles.page}>
    <header className={styles.header}>
      <div><span className="eyebrow">Fechamento</span><h1>Redução da Dia de Sorte.</h1><p>Escolha um pool de dezenas maior que os 7 sorteados. O algoritmo monta um conjunto de jogos de 7 dezenas garantindo uma pontuação mínima, sempre que os 7 sorteados caírem dentro do seu pool.</p></div>
    ...
    <div className={styles.layout}>
      <div className={styles.controls}>
        <section className={styles.card}>
          ...
          <div className={styles.segment}>{guarantees.map((entry) => <button type="button" key={entry.value} aria-pressed={guarantee === entry.value} className={guarantee === entry.value ? styles.active : ""} onClick={() => chooseGuarantee(entry.value)}>{entry.label}</button>)}</div>
          <label className={styles.field}>Dezenas no pool<select value={poolSize} onChange={(event) => { setPoolSize(Number(event.target.value)); setPool([]); setError(null); }}>{poolSizes.map((size) => <option key={size} value={size}>{size} dezenas</option>)}</select></label>
        </section>
        <section className={styles.card}>
        ...
        <section className={styles.card}>
          ...
          <p>Se as 7 dezenas sorteadas caírem todas dentro do seu pool de {poolSize}, ao menos 1 dos jogos gerados vai bater no mínimo <strong>{guarantee} pontos</strong> — cobertura combinatória provada, não estimativa. O algoritmo usa busca gulosa: reduz bastante o número de jogos frente à cobertura total, mas não é garantido ser o menor fechamento matematicamente possível.</p>
        </section>
    ...
    {rounds.length > 0 && <section ref={resultsRef} className={styles.roundList}>
      {rounds.map((round, position) => <section className={styles.results} key={round.id}>
        <div className={styles.resultHeading}>
          <div><span className="eyebrow">Redução {rounds.length - position}</span><h2>{round.tickets.length} jogos · garante {round.guarantee} pontos · pool {round.pool.map(pad).join(", ")}</h2></div>
