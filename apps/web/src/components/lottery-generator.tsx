...
function exclusionGroups(slug: LotterySlug) {
  ...
  const groups: { title: string; items: { key: string; label: string }[]; block?: boolean }[] = [
    { title: "Quadrantes", items: Array.from({ length: 4 }, (_, index) => ({ key: `quadrant:${index + 1}`, label: `Q${index + 1}` })) },
...
function Board({ slug, numbers, general = new Set<string>(), personal = new Set<string>(), fixed = [], avoided = new Set<number>(), lotomaniaView = "cross" }: {
  ...
  return <div className={`${styles.board} ${slug === "lotofacil" ? styles.lotofacilBoard : ""} ${canExclude(slug) && !blocks ? styles.quadrants : ""} ${blocks ? styles.blockBoard : ""}`} style={{ "--columns": game.columns } as CSSProperties} role="img" aria-label={`Volante: ${numbers.map(pad).join(", ")}`}>
    {blocks
...
export function LotteryGenerator({ histories, initialSlug }: { histories: Record<string, DrawNumbers[]>; initialSlug?: string }) {
  ...
  return <div className={styles.hub} style={{ "--generator-accent": lotteryGames[slug].color } as CSSProperties}>
    <div className={styles.selectorBar}><label htmlFor="generator-lottery">Modalidade</label><select id="generator-lottery" value={slug} onChange={(event) => setSlug(event.target.value as LotterySlug)}>{slugs.map((entry) => <option value={entry} key={entry}>{lotteryGames[entry].name}</option>)}</select><span>Um gerador para cada volante</span></div>
    {slug === "mais-milionaria" ? <MilionariaGenerator history={histories[slug] ?? []} /> : <StandardGenerator key={slug} slug={slug} history={histories[slug] ?? []} />}
  </div>;
}
...
function StandardGenerator({ slug, history }: { slug: LotterySlug; history: DrawNumbers[] }) {
  ...
  const [randomBlockCount, setRandomBlockCount] = useState(3);
  ...
  function toggleRule(key: string) {
    ...
    setRules(next); setRandomBlocks({}); invalidate();
  }
  ...
  return <main className={styles.page}>
    <header className={styles.header}><div><span className="eyebrow">Gerador de jogos</span><h1>Monte sua {game.name}.</h1><p>Leia as dezenas, marque seus palpites e confira cada cartela no formato do volante.</p></div><span className={styles.badge}>{game.total} números · {game.min} a {game.max} por jogo · {history.length} concursos na base</span></header>

    <div className={styles.layout}>
      <div className={styles.controls}>
        {slug === "lotofacil" && <section className={styles.card}><h2>Tipo de jogada</h2><div className={styles.segment}><button type="button" className={!mirror ? styles.active : ""} onClick={() => { setMirror(false); invalidate(); }}>Jogos normais</button><button type="button" className={mirror ? styles.active : ""} onClick={() => { setMirror(true); invalidate(); }}>Jogada espelho</button></div><p className={styles.helper}>No espelho, cada par compartilha 5 fixas. As outras 20 dezenas são divididas entre A e B: juntas, as duas cartelas cobrem as 25 dezenas.</p></section>}
        {slug === "lotomania" && <section className={styles.card}>
          ...
          <div className={styles.segment}>
            ...
            <button type="button" className={lotomaniaMirrorMode ? styles.active : ""} onClick={() => { setLotomaniaMirrorMode(true); setQuantity((current) => Math.min(current, 10)); setSelectedGame((current) => Math.min(current, 9)); setRandomBlocks({}); invalidate(); }}>Com espelho</button>
          </div>
          <p className={styles.helper}>O gerador prefere até 3 dezenas por bloco 2 × 2 e só completa um bloco quando as restrições exigem. No espelho, um bloco vazio na base fica completo na cartela complementar.</p>
        </section>}
        ...
        : <><section className={styles.card}>
          ...
          <div className={styles.fields}>
            <label className={styles.field}>{lotomaniaMirrorMode ? "Quantidade de pares" : "Quantidade"}<select value={quantity} onChange={(event) => { const next = Number(event.target.value); setQuantity(next); setSelectedGame((current) => Math.min(current, next - 1)); setRandomBlocks({}); invalidate(); }}>{Array.from({ length: lotomaniaMirrorMode ? 10 : 20 }, (_, index) => <option key={index} value={index + 1}>{index + 1} {lotomaniaMirrorMode ? (index ? "pares" : "par") : (index ? "jogos" : "jogo")}</option>)}</select></label>
          ...
          {size > game.min && <p className={styles.helper}>Uma cartela com {size} dezenas custa mais do que a aposta simples. Confira o valor na CAIXA antes de apostar.</p>}
        </section>
        <section className={styles.card}><div className={styles.sectionTitle}><span>02</span><div><h2>Escolha o critério</h2><p>Você pode comparar escolhas baseadas no histórico.</p></div></div><div className={styles.modeGrid}>{modes.map((entry) => <button type="button" key={entry.value} disabled={!history.length && entry.value !== "pure"} className={mode === entry.value ? styles.modeActive : ""} aria-pressed={mode === entry.value} onClick={() => { setMode(entry.value); invalidate(); }}><strong>{entry.label}</strong><small>{entry.detail}</small></button>)}</div>{history.length ? <label className={styles.field}>Repetidas do último concurso #{latest.contest}{lotomaniaMirrorMode ? " na base" : ""}<select value={repeatCount ?? "any"} onChange={(event) => { setRepeatCount(event.target.value === "any" ? null : Number(event.target.value)); invalidate(); }}><option value="any">Sem quantidade fixa</option>{Array.from({ length: Math.min(size, game.drawSize) + 1 }, (_, index) => <option value={index} key={index}>{index} repetidas</option>)}</select></label> : <p className={styles.helper}>Ainda não há resultados na base para aplicar critérios históricos ou repetições.</p>}</section>
        {canExclude(slug) && <section className={styles.card}>
          ...
          {slug === "lotomania" && <div className={styles.randomBlocks}>
            ...
            <p className={styles.helper}>Escolha quantos blocos adicionais excluir em cada cartela-base. Os blocos manuais continuam valendo; repetir o sorteio troca apenas os automáticos. Se mudar as áreas manuais, sorteie novamente.</p>
            <div><label className={styles.field}>Blocos por {lotomaniaMirrorMode ? "par" : "jogo"}<select value={randomBlockCount} onChange={(event) => setRandomBlockCount(Number(event.target.value))}>{Array.from({ length: 12 }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1} {index ? "blocos" : "bloco"}</option>)}</select></label><button type="button" onClick={drawRandomBlocks}>Sortear para {quantity} {lotomaniaMirrorMode ? quantity === 1 ? "par" : "pares" : quantity === 1 ? "jogo" : "jogos"}</button></div>
            {Object.keys(randomBlocks).length > 0 && <><p className={styles.helper}>Sorteados: {Array.from({ length: quantity }, (_, index) => `${lotomaniaMirrorMode ? "par" : "jogo"} ${index + 1}: ${(randomBlocks[index] ?? []).map((key) => key.replace("block:", "B")).join(", ") || "nenhum"}`).join(" · ")}</p><button className={styles.clear} type="button" onClick={() => { setRandomBlocks({}); invalidate(); }}>Limpar blocos sorteados</button></>}
          </div>}
          ...
          <button type="button" className={styles.clear} onClick={() => { setRules((current) => scope === "general" ? { ...current, general: [] } : { ...current, personal: { ...current.personal, [selectedGame]: [] } }); setRandomBlocks({}); invalidate(); }}>Limpar {scope === "general" ? "regras gerais" : `${lotomaniaMirrorMode ? "par" : "jogo"} ${selectedGame + 1}`}</button>
        </section>}</>}
      ...
      <aside className={styles.preview}>
        ...
        <section className={styles.actionBox}>
          ...
          {!mirror && history.length > 0 && <button type="button" className={styles.suggestion} onClick={() => { setMode("mixed"); generate("mixed"); }}>Sugestão Nexo com histórico</button>}
          <small>{lotomaniaMirrorMode ? "O gerador minimiza blocos vazios ou completos no par. Fixas, evitadas e exclusões se aplicam à base; o espelho usa o complemento exato." : slug === "lotomania" ? "O gerador minimiza blocos completos, mas permite os necessários para chegar às 50 dezenas." : "Suas dezenas fixas e evitadas são respeitadas em todos os jogos normais."}</small>
        </section>
      </aside>
    </div>
    ...
    {tickets.length > 0 && <section ref={resultsRef} className={styles.results} aria-live="polite">
      ...
      <HistoricalBacktest key={JSON.stringify(tickets)} slug={slug} tickets={tickets} />
      <div className={styles.ticketGrid}>{tickets.map((ticket, index) => <article className={styles.ticket} key={`${index}-${ticket.numbers.join("-")}`}>
        <div><strong>{mirror ? `Par ${Math.floor(index / 2) + 1} · Cartela ${index % 2 ? "B" : "A"}` : lotomaniaMirrorMode ? `Par ${Math.floor(index / 2) + 1} · ${index % 2 ? "Espelho" : "Base"}` : `Jogo ${index + 1}`}</strong><small>{ticket.numbers.length} dezenas{slug === "lotomania" ? ` · ${lotomaniaBlockCounts(ticket.numbers).filter((count) => count === 4).length} blocos completos` : ""}</small></div>
